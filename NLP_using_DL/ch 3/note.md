# CHAPTER 3 STUDY (2026.8.6-2026.9.18)
...
## 3-4 한국어에서의 언어 모델
### 1. 한국어는 어순이 중요하지 않다.
한국어에서 어순이 중요하지않다. 즉, 이전 단어가 주어졌을때, 다음 단어가 나타날 확률을 구하는데 어순이 중요하지 않다는 뜻으로서 다음 단어로 어떤 단어든 등장할 수 있다는 의미이다.
```
1. 나는 운동을 합니다 체육관에서
2. 나는 체육관에서 운동을 합니다.
3. 체육관에서 운동을 합니다.
...
위의 4개의 문장은 전부 의미가 통한다. 심지어 '나는'이라는 주어가 생략되어도 말이 된다. 이렇듯 한국어는 단어 순서가 뒤죽박죽이어도 의미가 전달되기때문에 확률에 기반한 언어 모델이 제대로 다음 단어를 예측하기 어렵다.  

### 2. 한국어는 교착어이다.
교착어는 한국어에서의 언어 모델 작동을 어렵게 만든다. 띄어쓰기 단위인 어절 단위로 토큰화를 할 경우에는 문장에서 발생가능한 단어의 수가 늘어난다. 예시로 교착어인 한국어에는 동사의 주어나 목적어를 위해서 조사가 존재한다.  

### 3. 한국어는 띄어쓰기가 제대로 지켜지지 않는다.
한국어는 띄어쓰기를 제대로 하지않아도 의미가 전달되며, 띄어쓰기 규칙 또한 상대적으로 까다로운 언어이기때문에 자연어 처리를 하는데있어서 한국어 코퍼스는 띄어쓰기가 지켜지지 않은 경우가 많다. 그렇기때문에 토큰이 제대로 분리 되지 않은채 훈련 데이터로 사용된다면 언어 모델은 제대로 동작하지 않는다.  

## 3-5 Perplexity
모델 내에서 자신의 성능을 수치화하여 결과를 내놓는 방법을 perplexity라고 한다.  

### 1. 언어 모델의 평가 방법(Evaluation metric) : PPL
perplexity는 언어 모델을 평가하기 위한 평가 지표로 PPL이라고 표현한다.
PPL은 수치가 **낮을수록** 언어 모델의 성능이 좋다는 것을 의미한다.  

PPL은 문장의 길이로 정규화된 문장 확률의 역수이다. 문장 W의 길이가 N이라고 가정했을 때 식은 아래와 같다.
$$PPL(W)=P(w_{1}, w_{2}, w_{3}, ... , w_{N})^{-\frac{1}{N}}=\sqrt[N]{\frac{1}{P(w_{1}, w_{2}, w_{3}, ... , w_{N})}}$$
chain rule을 적용하면 아래와 같다.
$$PPL(W)=\sqrt[N]{\frac{1}{P(w_{1}, w_{2}, w_{3}, ... , w_{N})}}=\sqrt[N]{\frac{1}{\prod_{i=1}^{N}P(w_{i}| w_{1}, w_{2}, ... , w_{i-1})}}$$
n-gram도 적용할 수 있다. bigram 언어 모델은 아래와 같다.
$$PPL(W)=\sqrt[N]{\frac{1}{\prod_{i=1}^{N}P(w_{i}| w_{i-1})}}$$

### 2. 분기 계수(Branching factor)
분기 계수 : PPL을 선택할 수 있는 경우의 수  

만약 언어 모델에 테스트 데이터를 주고 측정했더니 PPL이 10이 나왔다고 가정하면, 해당 언어 모델은 다음 단어를 예측하는 모든 시점마다 평균 10개의 단어 중 무엇이 정답인지 고민한다고 볼 수 있다. 같은 테스트 데이터에서 두 언어 모델의 PPL을 각각 계산했을 때 PPL이 더 낮은 언어 모델의 성능이 더 좋다고 볼 수 있다.
$$PPL(W)=P(w_{1}, w_{2}, w_{3}, ... , w_{N})^{-\frac{1}{N}}=(\frac{1}{10}^{N})^{-\frac{1}{N}}=\frac{1}{10}^{-1}=10$$
단, 평가 방법에서 주의할 점은 PPL 값이 낮다는 것은 데이터상 정확도가 높다는 것이지, 사람이 직접 느끼기에 좋은 모델을 의미하지 않는다. 또한 언어 모델의 PPL은 테스트 데이터에 의존하므로 두 개 이상의 언어 모델을 비교할 때는 양이 많고, 도메인에 알맞은 동일한 테스트 데이터를 사용해야 신뢰도가 높다.

## 3-6 조건부 확률(Conditional Probability)
![image](./img/조건부_확률.webp)
```
A = 학생이 남학생일 때
B = 학생이 여학생일 때
C = 학생이 중학생일 때
...
2. 학생을 뽑았을 때, 고등학생인 남학생일 확률
$$P(A∩D)=80/360=2/9=0.222...$$

3. 고등학생 중 한명을 뽑았을 때, 남학생일 확률
$$P(A|D)=P(A∩D)/P(D)=(80/360)(200/360)=80/200=0.4